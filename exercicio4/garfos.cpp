...
static void DFS_aux(ListaAdjacencia gf, std::vector<bool> &visited, ulong pos)
{
    visited[pos] = true;
...
// Ler arquivo pajeck pra um grafo bidirecional
...
static void populalst(FILE *fptr, ListaAdjacencia &lst)
{
    ...
    do
    {
        ...
        lst.addAresta(vert1, vert2);
    } while (!feof(fptr));
}


static bool possuiCicloAux(ListaAdjacencia gf, std::vector<bool> &visited, ulong pos)
{
    visited[pos] = true;
    ...
    for (int i = 0; i < gf.estrutura[pos].size(); i++)
    {
        if (visited[gf.estrutura[pos][i]] == false)
        {
            return possuiCicloAux(gf, visited, gf.estrutura[pos][i]);
        }else{
            return true;
        }
    }
...
bool possuiCiclo(ListaAdjacencia lst){
    std::vector<bool> vs(lst.numVertices, false);

    return possuiCicloAux(lst, vs, 1);
}
